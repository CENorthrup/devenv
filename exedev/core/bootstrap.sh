...
printf '%s ALL=(ALL) NOPASSWD:ALL\n' "$user_name" > "/etc/sudoers.d/90-devenv-$user_name"
chmod 440 "/etc/sudoers.d/90-devenv-$user_name"
...
sudo -u "$user_name" env HOME="$user_home" USER="$user_name" LOGNAME="$user_name" \
